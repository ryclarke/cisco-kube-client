...
function getNewToken(config, next) {
    return new Promise(function (resolve, reject) {
        ...
        // Do not preserve user credentials after first authentication
        ...
        request(options, function (error, response) {
            ...
            if (error) {
                ...
                if (error.statusCode == 401) {
                    errors.throw(config.log.fatal, error, null, '401 Unauthorized', reject);
                } else {
                    errors.throw(config.log.fatal, error, null, null, reject);
                }
                reject(error);
            } else {
                try {
