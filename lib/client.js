...
function KubernetesClient(options) {
    ...
    // Initialize client Bunyan logger
    var bunyan_log = bunyan.createLogger({ name: 'cisco-kube-client', level: options.loglevel || bunyan.FATAL });
    ...
    try {
        ...
        bunyan_log.debug('client initialization options parsed');
    } catch (error) {
        bunyan_log.fatal(error);  // Log client configuration errors
        throw error;
    }
    /**
     * @private
     * @name log
     * @memberof module:client.KubernetesClient#config
     * @description Bunyan logger for the Client
     * @type {bunyan}
     */
    Object.defineProperty(this.config, 'log', { value: bunyan_log });
