...
function Endpoint(client, resource, nested, options) {
     ...
     * @type {bunyan}
     */
    Object.defineProperty(this, '_log', { value: this.client.log.child({
    ...
    //this.client.log.trace('endpoint created: '
    //    + getPath(this.options.prefix, this.options.version, this.resource, this.options.child));
}
...
function baseRequest(self, method, query, body, opts, next) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        self.client.authenticate(false).then(function (config) {
                        ...
                        self.client.authenticate(true).then(function (config) {
                            ...
                            request(opts, function (error, response, body) {
                                ...
                                if (error) {
                                    ...
                                    self._log.error(Object.merge({Authentication: '********'}, opts, true, false));
                                    reject(error);
                                } else {
                            ...
                            self._log.error(JSON.stringify(error));
                            throw error;
                        });
                    } else {
                        if (error.code === 'ENOTFOUND') {
                            self._log.fatal(error);
                            self._log.fatal(Object.merge({Authentication: '********'}, opts, true, false));
                        } else {
                            self._log.error(error);
                            self._log.error(Object.merge({Authentication: '********'}, opts, true, false));
                        }
